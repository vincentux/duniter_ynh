...
url=$(curl -s https://github.com/duniter/duniter/releases/download/v1.5.1/duniter-server-v1.5.1-linux-armv7l.deb | grep "browser_" | grep $arch | grep "linux" | grep "server" | sort -r | head -1 | cut -d\" -f4)
